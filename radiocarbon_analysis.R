...
# 6. Set up for correlations (with charcoal)
...
set.seed(42)
ncores = 4 #Enter number of cores available
...
vario_fit_gau <- gstat::fit.variogram(vario, gstat::vgm(c("Exp","Mat","Sph","Ste","Gau",'Exc','Log','Spl')), fit.kappa = T) #Fit variogram best
vario_fit_ste <- gstat::fit.variogram(vario, gstat::vgm(c("Exp","Mat","Sph","Ste",'Exc','Log','Spl')), fit.kappa = T) #Fit variogram 2nd best
vario_fit_mat <- gstat::fit.variogram(vario, gstat::vgm(c("Exp","Mat","Sph",'Exc','Log','Spl')), fit.kappa = T) #Fit variogram 3rd best
vario_fit_sph <- gstat::fit.variogram(vario, gstat::vgm(c("Exp","Sph",'Exc','Log','Spl')), fit.kappa = T) #Fit variogram 4th best

vario_sserr <- attr(vario_fit, "SSErr")
vario_sserr_gau <- attr(vario_fit_gau, "SSErr")
...
krige_neo_gau <- gstat::krige(layer~1, neo_data_xyz, as(iberia_map_exp_25830_rast_small, "SpatialPixels"), model=vario_fit_gau) #Perform kriging based on model and interpolation raster
krige_neo_raster_gau <- raster::raster(krige_neo_gau) %>%  #Generate raster
  raster::mask(iberia_map_exp_25830)
...
max_krige_neo_raster_gau <- max(krige_neo_raster_gau@data@values, na.rm = T)
min_krige_neo_raster_gau <- min(krige_neo_raster_gau@data@values, na.rm = T)
...
vario_preds_gau <- gstat::variogramLine(vario_fit_gau, maxdist = max(vario$dist)) %>% 
  dplyr::mutate(Model = paste0("Gaussian", ": ", round(vario_sserr_gau))) #Add fit standard error
...
vario_models <- vario_preds %>% #Combine model outputs for plotting
  rbind(vario_preds_gau, vario_preds_ste, vario_preds_mat, vario_preds_sph)
...
raster::writeRaster(krige_neo_raster, "other_output/pop/kriging_neoloithic.grd") #To be used in neolithic_analysis.R

#Plot kriging
lon_lat_krige_neo_raster <- raster::projectRaster(krige_neo_raster, crs = 4258) #Change projection to spherical
lon_lat_krige_neo_raster_gau <- raster::projectRaster(krige_neo_raster_gau, crs = 4258) #Change projection to spherical
...
pdf("figs/pop/Neo/Kriging/lon_lat_krige_neo_raster_gau.pdf", height = 3.15, width = 4.5, pointsize = 9)
par(mar = c(2, 2, 1, 1))
raster::plot(lon_lat_krige_neo_raster_gau, col = hcl.colors(20, "Viridis"), colNA = "aliceblue")
dev.off()
...
# 6. Correlation work with ROC bins
# ---------------------------------------------------------
# ---------------------------------------------------------
breakpoints_roc <- seq(9950, 3550, -100) #To allign with centre of bins from charcoal analysis
blocks_roc_SPD <- rcarbon::spd2rc(spd, breaks = breakpoints_roc, backsight = NULL, changexpr = expression((t1/t0)^(1/d) - 1)) #ROC for defined blocks
roc_blocks <- tibble(age = seq(9800, 3600, -100), roc = blocks_roc_SPD$roca) %>% 
  dplyr::mutate(roc = scale(roc))
rio::export(roc_blocks, "./other_output/pop/roc_blocks.csv") #To be used in correlation analysis
