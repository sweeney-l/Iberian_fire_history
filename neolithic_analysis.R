...
neo_min_est_age <- min(neo_dat$EST_AGE)
...
neo_Zbin_dat <- neo_Zbinning(dt = neo_dat, bins = seq(neo_min_est_age,(neo_min_est_age+12000), neo_bw), binhw = neo_bw/2) #12000 because want next 12k data from first date


# Give ages of each bin as a column
neo_bin_age <- seq(neo_min_est_age,(neo_min_est_age+12000),neo_bw)
...
neo_targend <- neo_min_est_age+12000 #12000 so that 0 to 12000 of the original data is included
neo_targstep <- neo_bw 
...
par(mar = c(4, 4, 1, 1))
plot(neo_x, neo_y, xlab="Age before and after the start of the Neolithic", ylab="Z-scores of transformed charcoal influx", xlim=c(750,-750), ylim=c(-1,1), type="n", mgp = c(2.4, 1, 0), cex.lab = 1.2, xaxt = "n") ##make sure xlim is right.
...
neo_fitname <- paste("neo_Zlocfit_",as.character(neo_hw), sep="")
colnames(neo_loc01_fit) <- c("neo_age", neo_fitname)
lines(neo_loc01_fit[,1], neo_loc01_fit[,2], lwd=2, col="red") 
...
# Set up
nreps_neo <- 100 #how many times a date is selected
p_vario_error <- c(0) #To help catch when an error occurs

p_all_neo_curveout <- data.frame(matrix(, nrow=121, ncol=0)) #Dataframe to store output
p_all_neo_curveout_loc <- data.frame(matrix(, nrow=121, ncol=0)) #Dataframe to store output
...
for (q in 1:nreps_neo){
  ...
  p_neo_min_est_age <- min(p_neo_dat$EST_AGE)
  ...
  p_neo_Zbin_dat <- p_neo_Zbinning(dt = p_neo_dat, bins = seq(neo_min_est_age,(neo_min_est_age+12000), p_neo_bw), binhw = p_neo_bw/2) # 12000 because want next 12k data from first date. Note standardised bin here
  
  # Give ages of each bin as a column
  p_neo_bin_age <- seq(neo_min_est_age,(neo_min_est_age+12000),p_neo_bw)
  ...
  # Target ages for fitted values
  p_neo_targbeg <- neo_min_est_age #note standardised to neo to ensure comparability
  p_neo_targend <- neo_min_est_age+12000
  ...
  p_neo_fitname <- paste("p_neo_Zlocfit_",as.character(p_neo_hw), sep="")
  colnames(p_neo_loc01_fit) <- c("p_neo_age", neo_fitname)
  
  
  #Output
  p_neo_curveout <- data.frame(cbind(p_neo_targage.df$p_neo_x, p_neo_pred01$fit, neo_ndec_tot, neo_xspan, neo_ninwin_tot))
  colnames(p_neo_curveout) <- c("age", paste0("locfit_", q), "nsites", "window", "ninwin")
  p_neo_curveout
...
p_neo_loc_95 <- dplyr::tibble(key = "p_neo_loc_95", value = apply(p_all_neo_curveout_loc, 1, function(x) quantile(x,prob=0.975, na.rm=T))) #Calculate 95% CI
p_neo_loc_05 <- dplyr::tibble(key = "p_neo_loc_05", value = apply(p_all_neo_curveout_loc, 1, function(x) quantile(x,prob=0.025, na.rm=T))) #Calculate 95% CI
p_neo_loc <- dplyr::tibble(key = "p_neo_loc", value = neo_loc01_fit$neo_Zlocfit_500)

# p_neo_loc_plot <- reshape2::melt(p_all_neo_curveout_loc)   
p_neo_loc_plot <- tidyr::gather(p_all_neo_curveout_loc) %>% 
  rbind(p_neo_loc) %>% 
  rbind(p_neo_loc_95) %>% 
  rbind(p_neo_loc_05) %>%     
  dplyr::mutate(age = rep(p_all_neo_curveout$age, times = (ncol(p_all_neo_curveout_loc)+3))) %>%  
  dplyr::mutate(size = c(rep(1, times = 121*(ncol(p_all_neo_curveout_loc))), rep(2, times = 121), rep(1, times = 121), rep(1, times = 121))) %>% 
  dplyr::mutate(alpha = c(rep(0.3, times = 121*(ncol(p_all_neo_curveout_loc))), rep(2, times = 121), rep(1, times = 121), rep(1, times = 121)))


# Plot showing ranges of output from small variations in Neolthic tagged calibration dates
ggplot2::ggplot(data = p_neo_loc_plot, mapping = aes(y = value, x = age, color = key, size = size, alpha = alpha))+
  geom_line()+
  ...
  coord_cartesian(xlim = c(750,-750), ylim = c(-.75, 0.2))+
