...
par(mar = c(4, 4, 1, 1))
plot(neo_x, neo_y, xlab="Age before and after the start of the Neolithic", ylab="Charcoal z-score composite", xlim=c(750,-750), ylim=c(-1,1), type="n", mgp = c(2.4, 1, 0), cex.lab = 1.2, xaxt = "n") ##make sure xlim is right.
...
## Neo curve plotting: detrended

# Curve output path and file
neo_curvecsvpath <- "./figs/neo_rpd/"
neo_curvefile <- paste(neo_curvecsvpath,"neo_rpd_detrend",".csv", sep="") #adjust to particular data you use now. .
print(neo_curvefile)

# .pdf plot of bootstrap iterations
if (plotout == "pdf") { 
  neo_pdffile <- paste(neo_curvecsvpath,"neo_rpd_detrend", ".pdf", sep="")
  print(neo_pdffile)
}
...
# Step 3 -- Plot the unresampled (initial) fit
neo_fitname <- paste("neo_detrend_locfit_",as.character(neo_hw), sep="")
colnames(detrend_neo_loc01_fit) <- c("neo_age", neo_fitname)
...
# 5. Permutation work based on variable Neolithic dates (based on non_detrended data)
# ---------------------------------------------------------
# ---------------------------------------------------------
...
for (q in 1:nreps_neo){
  ...
  p_neo_curveout <- data.frame(cbind(p_neo_targage.df$p_neo_x, p_neo_loc01_fit$p_neo_locfit_500, neo_ndec_tot, neo_xspan, neo_ninwin_tot))
...
p_neo_loc <- dplyr::tibble(name = "p_neo_loc", value = neo_loc01_fit$neo_locfit_500)
...
ggplot2::ggplot(data = p_neo_loc_plot, mapping = aes(y = value, x = age, color = name, size = size, alpha = alpha))+
  ...
  ggplot2::labs(x = "Age before and after the start of the Neolithic", y = "Charcoal z-score composite")
...
ts_neo_loc_plot_annual %>% 
  feasts::gg_tsdisplay(difference(difference(loc_annual)), plot_type='partial')

char_model_arima <- ts_neo_loc_plot_annual %>% #Generate ARIMA model based on data prior to Neolithic
  fabletools::model(fable::ARIMA(loc_annual, trace = T, stepwise = F, approximation = F))
