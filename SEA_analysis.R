...
first_sea <- 7395 # Manually select break points based on SPD pop data
...
sea_window <- 1000 # Set range of data to be considered for each epoch. Note, these should not overlap otherwise relationship becomes blurred
...
par(mar = c(4, 4, 1, 1))
plot(sea_x, sea_y, xlab="Age before and after ~7400 and ~5400", ylab="Z-scores of transformed charcoal influx", xlim=c(750,-750), ylim=c(-1,1), type="n", cex.lab = 1.2, mgp = c(2.4, 1, 0), xaxt = "n") ##make sure xlim is right.
...
sea_fitname <- paste("sea_Zlocfit_",as.character(sea_hw), sep="")
colnames(sea_loc01_fit) <- c("sea_age", sea_fitname)
...
max_range_sea <- 200 
nreps_sea <- 100 #how many times a date is selected
...
for (z in 1:nreps_sea){ # Loop to re-run binning and composite curve generation (as above)
  ...
  p_sea_fitname <- paste("p_sea_Zlocfit_",as.character(p_sea_hw), sep="")
  colnames(p_sea_loc01_fit) <- c("p_sea_age", p_sea_fitname)
 
  #Output
  p_sea_curveout_both <- data.frame(cbind(p_sea_targage.df$p_sea_x, p_sea_pred01$fit, p_sea_ndec_tot, p_sea_xspan, p_sea_ninwin_tot))
  colnames(p_sea_curveout_both) <- c("age", paste0("locfit_", first_sea_rand, "_", second_sea_rand), "nsites", "window", "ninwin")
  p_sea_curveout_both
...
p_loc_95_both <- dplyr::tibble(key = "p_loc_95", value = apply(p_all_sea_curveout_loc_both, 1, function(x) quantile(x,prob=0.975, na.rm=T))) #Calculate 95% CI
p_loc_05_both <- dplyr::tibble(key = "p_loc_05", value = apply(p_all_sea_curveout_loc_both, 1, function(x) quantile(x,prob=0.025, na.rm=T))) #Calculate 95% CI
sea_loc <- dplyr::filter(sea_loc01_fit, dplyr::between(sea_age, -800, 800))
p_sea_loc <- dplyr::tibble(key = "p_sea_loc", value = sea_loc$sea_Zlocfit_500)

# p_all_sea_curveout_loc_both <- p_all_sea_curveout_loc_both %>% 
#   dplyr::relocate(1, .after = dplyr::last_col())
p_sea_plot_both <- tidyr::gather(p_all_sea_curveout_loc_both) %>%  
  rbind(p_sea_loc) %>% 
  rbind(p_loc_95_both) %>% 
  rbind(p_loc_05_both)  %>%   
...
ggplot2::ggplot(data = p_sea_plot_both, mapping = aes(y = value, x = age, color = key, size = size, alpha = alpha))+
  geom_line()+
  ...
  labs(x ="Age before and after ~7400 and ~5400", y ="Z-scores of transformed charcoal influx")+
  ...
  annotate("text", x = -300, y = 0.125, label = "Rapid population growth", color = "blue", size = 12*0.36, family = "Times")+
...
loc01_fit_annual_sea_adj <- dplyr::tibble(sea_age = -200:750) %>% 
  ...
  dplyr::mutate(sea_loc01_fit_annual = zoo::na.spline(sea_Zlocfit_500)) %>% #Interpolate annual
...
model_3 %>% 
  ...
  annotate("text", x = 150, y = -0.375, label = "Before", color = "blue", size = 12*0.36, family = "Times")+
  annotate("text", x = -450, y = -0.375, label = "After", color = "blue", size = 12*0.36, family = "Times")+
