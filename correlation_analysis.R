...
loc01_fit <- rio::import("./other_output/rpd/loc01_fit.csv") #Fitted charcoal composite data
...
# Define periods when charcoal and ROC population above or below quartiles
# Charcoal years
...
# Figures
# Charcoal locfit with times when ROC population above and below quartiles
ggplot2::ggplot(data = corr_char_rocsm, mapping= aes(y = char_locfit, x = age))+
  geom_line()+
...
# ROC population locfit with times when charcoal above and below
ggplot2::ggplot(data = corr_char_rocsm, mapping= aes(y = roc_locfit, x = age))+
  geom_line()+
  geom_rect(data = corr_char_rocsm[1,],
            aes(xmin = yr_char_above_75sm[1], xmax = yr_char_above_75sm[7], ymin = -Inf, ymax = Inf), alpha = 0.2, fill = "blue")+
  geom_rect(data = corr_char_rocsm[2,],
            aes(xmin = yr_char_above_75sm[8], xmax = yr_char_above_75sm[16], ymin = -Inf, ymax = Inf), alpha = 0.2, fill = "blue")+
  scale_x_reverse()+
...
ggplot2::ggplot(data = corr_char_rocsm, mapping= aes(y = roc_locfit, x = age))+
  geom_line()+
  geom_vline(xintercept = c(yr_char_below_25sm), linetype = "dashed", color = "red")+
  scale_x_reverse()
...
ggpubr::ggscatter(corr_char_rocsm, x = "roc_locfit", y = "char_locfit",  add = "reg.line", conf.int = T)+
  ggplot2::labs(x = "SPD rate of change", y = "Charcoal accumulation composite")+
...
ggpubr::ggscatter(corr_char_rocsm_above_75_char, x = "roc_locfit", y = "char_locfit",  add = "reg.line", conf.int = T)+
  ggplot2::labs(x = "SPD rate of change", y = "Z-score composite of charcoal influx")+
  ggpubr::stat_cor(size = 8)+
  ggpubr::labs_pubr(base_size = 20)
