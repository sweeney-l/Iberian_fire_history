...
comp_char_spd <- spd_data_corr_sm %>% 
    dplyr::mutate(char = char_smoothed$char)
char_plot <- ggplot2::ggplot(data = comp_char_spd, mapping = aes(age, char)) +
  geom_line()+
  ylim(-1, 0.5)+
  scale_x_reverse()+
  ggplot2::labs(x = "", y = "Z-scores of transformed charcoal influx")+
  theme(plot.margin = unit(c(4,4,0,1), "mm"), panel.background = element_rect(fill = "white", color = "black"), panel.grid = element_line(color = "gray", linetype = 3)) +
  theme(text = element_text(family = "Times", size = 12))+
  annotate("text", x=9500, y=0.37, label= "A") 
spd_plot <- ggplot2::ggplot(data = comp_char_spd, mapping = aes(age, spd)) +
  geom_line()+
  scale_x_reverse()+
  ggplot2::labs(x = "Years cal. BP", y = "Summed probability")+
  theme(plot.margin = unit(c(1,4,1,1), "mm"), panel.background = element_rect(fill = "white", color = "black"), panel.grid = element_line(color = "gray", linetype = 3)) +
  theme(text = element_text(family = "Times", size = 12))+
  annotate("text", x=9500, y=1.18, label= "B") 
spd_char_simple <- gridExtra::grid.arrange(char_plot, spd_plot)
ggplot2::ggsave("./figs/cor/spd_char_simple.pdf", height = 16, width = 11.28, units = "cm", plot = spd_char_simple)
