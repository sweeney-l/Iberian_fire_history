...
# 1. data_import.R 
# 2. charcoal_analysis.R (this script)
...
#Influx figures
influx_plots <- rio::import("other_output/rpd/RPD_trans_zt200_8200k.csv") %>% 
  dplyr::left_join(dplyr::select(rpd_data_input, ID_SITE, site_name, entity_name, macro_micro), by = "ID_SITE") %>% 
  dplyr::distinct() %>% 
  dplyr::arrange(site_name)
 
influx_plots_list <- list() #Generate empty list to populate
influx_plots_list <- lapply(c(unique(influx_plots$site_name)), function(x){ #Generate plots of each included site
  ggplot2::ggplot(data = dplyr::filter(influx_plots, site_name == x))+
    geom_line(mapping = aes(x = EST_AGE, y = INFLUX))+
    scale_x_reverse()+
    labs(x = "Years cal. BP",
         y = "Charcoal influx\n(particles/cm2/year)",
         title = paste0(x, "\n", "(",influx_plots$macro_micro[which(influx_plots$site_name == x)[1]], ")"))+
    theme(axis.title = element_text(size = 8, family = "Times"),
          plot.title = element_text(size = 8, family = "Times"),
...
influx_plots_1 <- gridExtra::grid.arrange(influx_plots_list[[1]], 
                        influx_plots_list[[2]], 
                        influx_plots_list[[3]],
                        influx_plots_list[[4]],
                        influx_plots_list[[5]],
                        influx_plots_list[[6]],
                        influx_plots_list[[7]],
                        influx_plots_list[[8]], ncol = 2)
ggplot2::ggsave("figs/rpd/influx_plots_1.pdf", plot = influx_plots_1, height = 20, width = 15, units = "cm")

influx_plots_2 <- gridExtra::grid.arrange(influx_plots_list[[9]], 
                                          influx_plots_list[[10]], 
                                          influx_plots_list[[11]],
                                          influx_plots_list[[12]],
                                          influx_plots_list[[13]],
                                          influx_plots_list[[14]],
                                          influx_plots_list[[15]],
                                          influx_plots_list[[16]], ncol = 2)
ggplot2::ggsave("figs/rpd/influx_plots_2.pdf", plot = influx_plots_2, height = 20, width = 15, units = "cm")

influx_plots_3 <- gridExtra::grid.arrange(influx_plots_list[[17]], 
                                          influx_plots_list[[18]], 
                                          influx_plots_list[[19]],
                                          influx_plots_list[[20]],
                                          influx_plots_list[[21]],
                                          influx_plots_list[[22]],
                                          influx_plots_list[[23]],
                                          influx_plots_list[[24]], ncol = 2)
ggplot2::ggsave("figs/rpd/influx_plots_3.pdf", plot = influx_plots_3, height = 20, width = 15, units = "cm")

influx_plots_4 <- gridExtra::grid.arrange(influx_plots_list[[25]], 
                                          influx_plots_list[[26]], 
                                          influx_plots_list[[27]],
                                          influx_plots_list[[28]],
                                          influx_plots_list[[29]],
                                          influx_plots_list[[30]],
                                          influx_plots_list[[31]],
                                          influx_plots_list[[32]], ncol = 2)
ggplot2::ggsave("figs/rpd/influx_plots_4.pdf", plot = influx_plots_4, height = 20, width = 15, units = "cm")
...
charcoal_site_summary <- rpd_data_input %>% 
    dplyr::filter(dplyr::between(INTCAL2020_median, 3450, 10050)) %>% 
