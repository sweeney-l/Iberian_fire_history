...
# 6. Generate curves based on alternatitive specifications
# 7. Changepoint analysis
# 8. Save data for correlation_analysis.R
...
for (j in records) { # Loop over unique records
  ...
  if (nsamp > 1) { # For those entities with more than one sample
    ...
    if (type[1] == "INFL") {         # Adopt influx values as they are, calculate concentration
      influx <- quant
      influx_source <- "data"
    ...
    else if (type[1] == "CONC") {    # Calculate influx, adopt concentration values as they are
      conc <- quant
      conc_source <- "data"
    ...
    else if (type[1] == "C0P0"){     # Assume quantity is concentration-like
      conc <- quant
      conc_source <- "C0P0"
...
grid()
legend(8000, -1.5, legend = c("500-year", "250-year", "1000-year"), col = c("red", "blue", "black"), title="Smoothing window (half) width", lty=1, lwd = 2, cex=1, box.lty=0)
...
plot(x, y, xlab="Years cal. BP", ylab="Z-scores of transformed charcoal influx", xlim=c(10000,3500), ylim=c(-3,1), type="n", cex.lab = 1.2, mgp = c(2.4, 1, 0)) ##make sure xlim is right.
grid()
legend(8000, -1.5, legend = c("100-year", "50-year", "200-year", "500-year"), col = c("red", "blue", "black", "green"), title="Binwidth", lty=1, lwd = 2, cex=1, box.lty=0)
...
multiple_comp_elevation <- multiple_comp %>%
  dplyr::filter(type %in% c("inc_high_elevation", "normal")) %>%
  dplyr::mutate(type = if_else(type == "inc_high_elevation", "including higher elevation sites", "excluding higher elevation sites"))
...
ggplot2::ggplot(data = multiple_comp_elevation, mapping = aes(x = age, y = locfit, color = type))+
  ggplot2::geom_line()+
  ylim(-3, 1)+
  scale_x_reverse()+
  ...
  theme(plot.margin = unit(c(4,4,1,1), "mm"), panel.background = element_rect(fill = "white", color = "black"), panel.grid = element_line(color = "gray", linetype = 3)) +
  theme(text = element_text(family = "Times", size = 12))+
  ggsave("figs/rpd/Composites_elevation.pdf", height = 8, width = 15, units = "cm") #Minmax or max
...
ggplot2::ggplot(data = multiple_comp_micro, mapping = aes(x = age, y = locfit, color = type))+
  ...
  theme(plot.margin = unit(c(4,4,1,1), "mm"), panel.background = element_rect(fill = "white", color = "black"), panel.grid = element_line(color = "gray", linetype = 3)) +
  theme(text = element_text(family = "Times", size = 12))+
  ggsave("figs/rpd/Composites_micro.pdf", height = 8, width = 15, units = "cm") #Minmax or max
...
# 8. Save data for fire_human_relationship_analysis.R
# ---------------------------------------------------------
